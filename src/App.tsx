...
function Home({ onNavigate }: { onNavigate: (s: Screen) => void }) {
  ...
  void getCurrentShopId().then((shopId) => {
...
function BookingScreen({
  ...
  const [dayWaiting, setDayWaiting] = useState(false);
...
function ConfirmScreen({ bookingId, onHome }: { bookingId: string; onHome: () => void }) {
  ...
  useEffect(() => {
    ...
    const confirmBooking = () => {
      // ✅ Проверяем, что initData загрузился
      if (!window.Telegram?.WebApp?.initData) {
        console.log('⏳ Ждём initData...');
        setTimeout(() => {
          if (isMounted) confirmBooking();
        }, 300);
        return;
      }
